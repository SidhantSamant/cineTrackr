...
//       const tmdbShowResponse = await tmdbClient.get(`/tv/${tmdbId}`);
//       const tmdbShow = tmdbShowResponse.data;

//       // Step 3: Get all seasons and their episodes from Trakt
//       const seasonsResponse = await traktClient.get(`/shows/${traktShowId}/seasons?extended=full`);
...
//           const episodesResponse = await traktClient.get(
...
//             episodesResponse.data.map(async (episode) => {
//               try {
//                 // Get TMDB episode data for images
//                 const tmdbEpisodeResponse = await tmdbClient.get(
